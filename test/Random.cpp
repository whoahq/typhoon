...
TEST_CASE("CRandom::dice", "[random]") {
    SECTION("generates numbers within range") {
        ...
        auto result3 = CRandom::dice(1, seed);
        REQUIRE(result3 == 0);

        auto result4 = CRandom::dice(2, seed);
        REQUIRE(result4 < 2);

        auto result5 = CRandom::dice(2, seed);
        REQUIRE(result5 < 2);

        auto result6 = CRandom::dice(10, seed);
        REQUIRE(result6 < 10);

        auto result7 = CRandom::dice(10, seed);
        REQUIRE(result7 < 10);

        auto result8 = CRandom::dice(10, seed);
        REQUIRE(result8 < 10);
    }
}
